async function load() {
  return {
    watches: await fetch("https://www.horlogeforum.nl/c/horlogemarkt/12").then((data) => data.text()).then((text) => new Promise((resolve) => {
      ...
      for (let i = 0; i < extractedUrls.length; i++) {
        resultsArray.push({
          url: extractedUrls[i],
          name: extractedNames[i] || "No Name"
          // If name is not available, use 'No Name'
        });
      }
      const filteredArray = resultsArray.filter((watch) => !watch.name.startsWith("Regels voor de")).filter((watch) => !watch.name.startsWith("Binnen 6 maanden")).slice(0, 10);
      const withPicture = Promise.all(filteredArray.map((watch) => {
